...
&НаКлиенте
Перем СчетчикПовторныхЗапусковУпавшегоШага;

&НаКлиенте
Перем МассивШаговВыполненныхПовторно;

&НаКлиенте
Перем ОстатокПопытокВыполненияПриНеуспехе;
...
&НаКлиенте
Перем ПрекратитьВыполнениВсехСценариев;
...
Процедура ВосстановитьНастройки(ЭтоLinux)
	...
	Если ТипЗнч(Настройки) = Тип("Структура") Тогда
		...
		Настройки.Свойство("ЗапускатьУпавшиеСценарииРаз", Объект.ЗапускатьУпавшиеСценарииРаз);
...
Процедура СохранитьНастройки()
	...
	Настройки.Вставить("ЗапускатьУпавшиеСценарииРаз", Объект.ЗапускатьУпавшиеСценарииРаз);
...
&НаСервере
Процедура ДобавитьСнипет(Знач Снипет,Знач ИмяФайла,БылиОшибки,ТзнТаблицаИзвестныхStepDefinition)
	...
	СтрТаблицаИзвестныхStepDefinition.Транзакция              = Снипет.Транзакция;	
...
Процедура ДобавитьСнипетыСервер(МассивСнипетовИзОбработки,ИмяФайла,БылиОшибки)
	...
	Для каждого Снипет Из МассивСнипетовИзОбработки Цикл
		ДобавитьСнипет(Снипет,ИмяФайла,БылиОшибки,ТзнТаблицаИзвестныхStepDefinition);
	КонецЦикла;
...
// {{ построение дерева известных шагов

&НаКлиенте
Процедура ФильтрДереваШаговПриИзменении(Элемент)
	ДобавитьСнипетыВДеревоШаговНаСервере();
	ПодключитьОбработчикОжидания("СвернутьДеревоИзвестныхШагов", 0.5, Истина);
КонецПроцедуры

&НаСервере
Процедура ДобавитьСнипетыВДеревоШаговНаСервере()
	...
	тДерево = ОбъектСервер.ДеревоИзвестныхШагов;
	тДерево.Строки.Очистить();
	
	//получить список каталогов фич
	КаталогиФич = ТабРезультат.Скопировать(, "ИмяФайла");
	КаталогиФич.Свернуть("ИмяФайла");
	КаталогиФич.Сортировать("ИмяФайла Возр");
	
	Для каждого КаталогФичи Из КаталогиФич Цикл
		МассивКаталогов = РазложитьСтрокуВМассивПодстрок(КаталогФичи.ИмяФайла, РазделительКаталогов, Истина);
		СтрокаКаталога = "";
		тСнипет = тДерево;
		Для каждого Каталог Из МассивКаталогов Цикл
			Если СтрокаКаталога = "" Тогда
				СтрокаКаталога = Каталог;
			Иначе
				СтрокаКаталога = СтрокаКаталога + РазделительКаталогов + Каталог;
			КонецЕсли;
			
			ОтборКаталогов = Новый Структура;
			ОтборКаталогов.Вставить("ПутьКФайлу", НРег(СтрокаКаталога));
			тДеревоНашелПуть = тДерево.Строки.НайтиСтроки(ОтборКаталогов, Истина);
			
			Если тДеревоНашелПуть.Количество() <> 1 Тогда
				тСнипет = тСнипет.Строки.Добавить();
				тСнипет.Наименование = Каталог;
				тСнипет.ПутьКФайлу   = НРег(СтрокаКаталога);
				Если Сред(Каталог, СтрДлина(Каталог) - 3) = ".epf" Тогда
					тСнипет.ТипКартинки  = 1;
					тСнипет.Значение     = СтрЗаменить(СтрокаКаталога, РазделительКаталогов + Каталог, "");
				Иначе
					тСнипет.ТипКартинки  = 0;
				КонецЕсли;
			Иначе
				тСнипет = тДеревоНашелПуть[0];
			КонецЕсли;
			
			Отбор = Новый Структура;
			Отбор.Вставить("ИмяФайла", СтрокаКаталога);
			ТабРезультат_ = ТабРезультат.НайтиСтроки(Отбор);
			
			Для каждого СтрСнипет Из ТабРезультат_ Цикл
				Снипет = Новый Структура;
				Снипет.Вставить("ПредставлениеТеста", СтрСнипет.ПредставлениеТеста);
				Снипет.Вставить("ИмяПроцедуры", СтрСнипет.СтрокаРеальнойПроцедуры);
				Снипет.Вставить("Параметры",    СтрСнипет.Параметры);
				Снипет.Вставить("Транзакция",   СтрСнипет.Транзакция);
				Снипет.Вставить("Снипет",       СтрСнипет.Id);
				//Снипет.Вставить("ТипКартинки",  3);
				
				ДобавитьСнипетВДеревоШаговНаСервере(тСнипет,Снипет, СтрСнипет.ИмяФайла)
			КонецЦикла;	
		КонецЦикла;
	КонецЦикла;
...
&НаСервере
Процедура ДобавитьСнипетВДеревоШаговНаСервере(тДерево, Снипет, ИмяФайла)

	тШаг = тДерево.Строки.Добавить();
	тШаг.Наименование = Снипет.ПредставлениеТеста;
	тШаг.ТипКартинки  = 3;
	
	Если ЗначениеЗаполнено(Снипет.ИмяПроцедуры) Тогда
		тИмяПроцедуры = тШаг.Строки.Добавить();
		тИмяПроцедуры.Наименование = "Имя процедуры";
		тИмяПроцедуры.Значение     = Снипет.ИмяПроцедуры;
		тИмяПроцедуры.ТипКартинки  = 100;
	КонецЕсли;

	Если ЗначениеЗаполнено(Снипет.ИмяПроцедуры) Тогда
		тИмяСнипета = тШаг.Строки.Добавить();
		тИмяСнипета.Наименование = "Снипет";
		тИмяСнипета.Значение     = Снипет.Снипет;
		тИмяСнипета.ТипКартинки  = 100;
	КонецЕсли;	

	Поз = Найти(Снипет.Снипет,"(");
	СтрПараметры = Сред(Снипет.Снипет, Поз+1);
	СтрПараметры = Лев(СтрПараметры,СтрДлина(СтрПараметры)-1);
	
	МассивПром = РазложитьСтрокуВМассивПодстрок(СтрПараметры, ",");
	Если МассивПром.Количество() > 0 Тогда
		тПараметры = тШаг.Строки.Добавить();
		тПараметры.Наименование = "Параметры";
		тПараметры.ТипКартинки  = 100;
		...
		Для Каждого Элем Из МассивПром Цикл
			...
			тИмяПараметра = тПараметры.Строки.Добавить();
			тИмяПараметра.Наименование = Элем;
			тИмяПараметра.Значение     = Тип;
			тИмяПараметра.ТипКартинки  = 100;
			
		КонецЦикла;		
	КонецЕсли;	
	
	Если ЗначениеЗаполнено(Снипет.Транзакция) Тогда
		тТранзакция = тШаг.Строки.Добавить();
		тТранзакция.Наименование = "Транзакция";
		тТранзакция.Значение     = Снипет.Транзакция;
		тТранзакция.ТипКартинки  = 100;
	КонецЕсли;

КонецПроцедуры

&НаКлиенте
Процедура СвернутьДеревоИзвестныхФич()
	ДеревоФормыСтроки = Объект.ДеревоИзвестныхШагов.ПолучитьЭлементы();
	СвернутьСтрокиДереваФич(ДеревоФормыСтроки);
КонецПроцедуры

&НаКлиенте
Процедура СвернутьСтрокиДереваФич(ДеревоФормыСтроки)
	Для Каждого Строка Из ДеревоФормыСтроки Цикл
		Если Строка.ТипКартинки = 1 Тогда
			ИдентификаторСтроки = Строка.ПолучитьИдентификатор();
			Элементы.ДеревоИзвестныхШагов.Свернуть(ИдентификаторСтроки);
			Продолжить;
		КонецЕсли;
		ИдентификаторСтроки = Строка.ПолучитьИдентификатор();
		Элементы.ДеревоИзвестныхШагов.Развернуть(ИдентификаторСтроки);
			
		ПодчиненныеСтроки = Строка.ПолучитьЭлементы();
		Если ПодчиненныеСтроки.Количество() > 0 Тогда
			СвернутьСтрокиДереваФич(ПодчиненныеСтроки);
		КонецЕсли;	 
	КонецЦикла;
КонецПроцедуры

&НаКлиенте
Процедура СвернутьДеревоИзвестныхШагов()
	ДеревоФормыСтроки = Объект.ДеревоИзвестныхШагов.ПолучитьЭлементы();
	СвернутьСтрокиДереваШагов(ДеревоФормыСтроки);
КонецПроцедуры

&НаКлиенте
Процедура СвернутьСтрокиДереваШагов(ДеревоФормыСтроки)
	Для Каждого Строка Из ДеревоФормыСтроки Цикл
		Если Строка.ТипКартинки = 3 Тогда
			ИдентификаторСтроки = Строка.ПолучитьИдентификатор();
			Элементы.ДеревоИзвестныхШагов.Свернуть(ИдентификаторСтроки);
			Продолжить;
		КонецЕсли;
		...
		ПодчиненныеСтроки = Строка.ПолучитьЭлементы();
		Если ПодчиненныеСтроки.Количество() > 0 Тогда
			СвернутьСтрокиДереваШагов(ПодчиненныеСтроки);
		КонецЕсли;	 
	КонецЦикла;
КонецПроцедуры
...
Процедура ЗаполнитьДеревоЗначенийПоМассивуСервер(МассивРезультатОбходаКаталогов,ДвДанныеvbFeatureReader,СтруктураПараметров)
	...
	ФайлvbFeatureReader = Новый Файл(Объект.КаталогИнструментов + "\vbFeatureReader.epf");
...
Функция СоздатьСтруктуруПараметровЗапуска()
	...
	Рез.Вставить("ЗапускатьУпавшиеСценарииРаз", 2);
	Возврат Рез;
КонецФункции
...
Процедура ЗапускВРежимеКоманднойСтроки()
	...
	Если StartFeaturePlayer = Истина Тогда
		...
		ЗнЗапускатьУпавшиеСценарииРаз = ПолучитьЗначениеПереданногоПараметра(СтрутктураJSON,"ЗапускатьУпавшиеСценарииРаз");
		Если ЗначениеЗаполнено(ЗнЗапускатьУпавшиеСценарииРаз) Тогда
			Объект.ЗапускатьУпавшиеСценарииРаз = ЗнЗапускатьУпавшиеСценарииРаз;
		КонецЕсли;	
		...
		Если ЗнДелатьОтчетВФорматеjUnit = Истина Тогда
			...
			Если ЗначениеЗаполнено(ЗнКаталогOutputjUnit) Тогда
				...
				Если Не ФайлСуществуетКомандаСистемы(Файл.ПолноеИмя, "ЗнКаталогOutputjUnit") Тогда
					СоздатьКаталогКомандаСистемы(ЗнКаталогOutputjUnit);
					ФайлПроверкаСуществования = Новый Файл(ЗнКаталогOutputjUnit);
		...
		Если ЗнДелатьОтчетВФорматеCucumberJson = Истина Тогда
			...
			Если ЗначениеЗаполнено(ЗнКаталогOutputCucumberJson) Тогда
				...
				Если Не ФайлСуществуетКомандаСистемы(Файл.ПолноеИмя, "ЗнКаталогOutputCucumberJson") Тогда
					СоздатьКаталогКомандаСистемы(ЗнКаталогOutputCucumberJson);
					ФайлПроверкаСуществования = Новый Файл(ЗнКаталогOutputCucumberJson);
					Если НЕ ФайлСуществуетКомандаСистемы(ФайлПроверкаСуществования.ПолноеИмя) Тогда
...
Процедура ПриОткрытии(Отказ)
	...
	МассивШаговВыполненныхПовторно = Новый Массив;
	
	СчетчикПовторныхЗапусковУпавшегоШага = 0;
		...
		Для каждого СтрДеревоСтроки Из ДеревоСтроки Цикл
			...
			Если СтрДеревоСтроки.Сценарий = Истина Тогда
				Если ТекИДСценария <> Неопределено Тогда
					...
					Если ТекИДСценария.Найти(СтрДеревоСтроки.ИдСтроки) <> Неопределено Тогда
	...
	Если СокрЛП(АдресСнипета) = "" Тогда
		...
		Если БылаОшибка Тогда
			...
			Если Объект.ОстановкаПриВозникновенииОшибки Тогда
				...
				ПрекратитьВыполнениВсехСценариев = Истина;
			КонецЕсли;	 
		КонецЕсли;
...
Процедура ПерейтиКВыполнениюСледующегоСценария()
	...
	Если ПрекратитьВыполнениВсехСценариев Тогда
	...
	ВыполнитьПовторныйЗапускСценарияПриНеудаче();
	
	ПодключитьОбработчикОжидания("ВыполнинтьСценарийАссинхронноТаймер",Объект.ИнтервалВыполненияШага,Истина);
КонецПроцедуры

&НаКлиенте
Процедура ВыполнитьПовторныйЗапускСценарияПриНеудаче()
	
	Если НеобходимоПовторноЗапуститьУпавшийСценарийПриНеудаче() Тогда
		ТекИД_СценарияВМассиве = ТекИД_СценарияВМассиве -1;
		СчетчикПовторныхЗапусковУпавшегоШага = СчетчикПовторныхЗапусковУпавшегоШага +1;
		Если СчетчикПовторныхЗапусковУпавшегоШага = Объект.ЗапускатьУпавшиеСценарииРаз Тогда
			МассивШаговВыполненныхПовторно.Добавить(ТекИД_СценарияВМассиве);
			СчетчикПовторныхЗапусковУпавшегоШага = 0;
		КонецЕсли;	
	КонецЕсли;	
...
&НаКлиенте
Функция НеобходимоПовторноЗапуститьУпавшийСценарийПриНеудаче() 
	
	ПовторятьСценарийПриОшибке = Объект.ЗапускатьУпавшиеСценарииРаз<>0;
	
	Если Не ПовторятьСценарийПриОшибке Тогда
		Возврат Ложь;
	КонецЕсли;	
	
	Если ТекИД_СценарияВМассиве  <= 0 Тогда
		ИДПредыдущегоСценария = 0;
		Возврат Ложь;
	Иначе
		ИДПредыдущегоСценария = ТекИД_СценарияВМассиве-1;
	КонецЕсли;	
	
	Если МассивШаговВыполненныхПовторно.Найти(ИДПредыдущегоСценария) <> Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;	
	
	
	ИД_СтрокиСценария = МассивСценариевДляВыполнения[ИДПредыдущегоСценария].СтрокаДерева;
	Строка_Сценария = МассивСтрокДереваДанныеФормы[ИД_СтрокиСценария];
	
	Если ПовторятьСценарийПриОшибке И Строка_Сценария.Статус = "Failed" Тогда
		Возврат Истина;
	КонецЕсли;	
...
Процедура ВыполнинтьСценарийАссинхронноТаймер()
	...
	Если (МассивСценариевДляВыполнения.Количество()-1) < ТекИД_СценарияВМассиве Тогда
		...
		СделатьСообщение("Все сценарии обработаны!");
		МассивШаговВыполненныхПовторно.Очистить();
...
Процедура ВыполнитьСценарии(ТекСценарий = Неопределено) Экспорт
	...
	ПрекратитьВыполнениВсехСценариев = Ложь;
	...
	Если (ТекСценарий = Неопределено) и (Объект.РежимВыполненияСценарияСВыбранногоШага) Тогда
		ИдСценарияПоСтрокеШага = ОпределитьИдСценарияПоСтрокеШага(СтрокаШагаСКоторогоНадоВыполнятьСценарий.ИдСтроки);
...
&НаКлиенте
Процедура ВыполнитьФичу(Команда)
	СтрокаДерева = Элементы.ДеревоТестов.ТекущиеДанные;
	Если СтрокаДерева = Неопределено Тогда
		Возврат;
	КонецЕсли; 
	
	Если СтрокаДерева.Фича <> Истина Тогда
		СтрокаДерева = НайтиСтрокуФичиЧерезРодителя(СтрокаДерева);
		Если СтрокаДерева = Неопределено Тогда
			Сообщить("Не найден строка фичи!");
			Возврат;
		КонецЕсли;	 
	КонецЕсли;	 
	
	МассивИД = ПолучитьМассивСценариевФичи(СтрокаДерева);
	ВыполнитьСценарии(МассивИД);
КонецПроцедуры
...
Функция ОткрытьНовыйСеансTestClientИлиПодключитьУжеСуществующийДляНесколькихКлиентов(ДанныеЗапускаКлиентаТестирования)
	...
	Если ПодключитьУжеЗапущенныйTestClient(ДанныеЗапускаКлиентаТестирования.ПортЗапускаТестКлиента) Тогда
		...
		Если Объект.ДелатьЛогВыполненияСценариевВЖР Тогда
			ЗаписатьЛогВЖРИнформация("НайденПодключенныйTestClient","порт=" + ДанныеЗапускаКлиентаТестирования.ПортЗапускаТестКлиента);
		КонецЕсли;	 
		Возврат Истина;
	КонецЕсли;
	...
	Попытка
		...
		Если Объект.ДелатьЛогВыполненияСценариевВЖР Тогда
			ЗаписатьЛогВЖРИнформация("ПодключениеTestClient","порт=" + ДанныеЗапускаКлиентаТестирования.ПортЗапускаТестКлиента);
		КонецЕсли;	 
...
Процедура ЗаполнитьДанныеТекущейИнфобазы()
	...
	КонецЕсли;
	ДанныеТекущейИнфобазы.Вставить("ПутьКПлатформе", СтрокаЗапуска1с);	
...
Процедура ЗапуститьКлиентаТестирования(ДанныеЗапускаКлиентаТестирования, ДопПараметр = "")
	...
	СтрокаЗапуска1с = ДанныеЗапускаКлиентаТестирования.ПутьКПлатформе;
	...
	Если Найти(НРег(СтрокаЗапуска), "1cv8c") > 0 Или ЭтоLinux Тогда
		ЗапуститьСистему(СтрокаЗапуска);
	Иначе // толстый клиент для win
		ИмяВременногоBAT = ПолучитьИмяВременногоФайла("bat");
		ЗТ = Новый ЗаписьТекста(ИмяВременногоBAT,КодировкаТекста.ANSI,,Истина); 
		ЗТ.ЗаписатьСтроку("chcp 1251");
		ЗТ.ЗаписатьСтроку(СтрокаЗапуска); 
		ЗТ.Закрыть();
...
Процедура ПодготовитьСценарийКВыполнению() Экспорт
	...
	ЗТ.ЗаписатьСтроку(СгенерированныйСценарий);
	ЗТ.Закрыть();
...
Процедура ДанныеКлиентовТестированияПередНачаломИзменения(Элемент, Отказ)
	...
	Если ТекущиеДанные.Имя = "Этот клиент" Тогда
		Сообщить("Редактирование данного клиента нельзя.");
		Отказ = Истина;
	КонецЕсли;
