...
Функция ПолучитьСтрокуЗапускаСборки(ПараметрыСборки)
	...
	СохранятьЗамер = Ложь;
	Если НРег(ПараметрыСборки["СохранятьЗамерПроизводительности"]) = "истина" Тогда
		СохранятьЗамер = Истина;
	КонецЕсли;	 
	
	
	Если СохранятьЗамер Тогда
		ПутьКудаСохранятьЗамер = ПараметрыСборки["ПутьКудаСохранятьЗамерПроизводительности"];
		ФайлПроверкаСуществования = Новый Файл(ПутьКудаСохранятьЗамер);
		Если НЕ ФайлПроверкаСуществования.Существует() Тогда
			Попытка
				СоздатьКаталог(ПутьКудаСохранятьЗамер);
			Исключение
		...
		ФайлПроверкаСуществования = Новый Файл(ПутьКудаСохранятьЗамер);
		Если НЕ ФайлПроверкаСуществования.Существует() Тогда
			ВызватьИсключение "Неверно указан параметр <" + ПутьКудаСохранятьЗамер + ">. Каталог " + ПутьКудаСохранятьЗамер + " не существует!";
		КонецЕсли;	 
	КонецЕсли;	 
	
	Если Не СохранятьЗамер Тогда
		Стр = Стр + " /Execute " + ПараметрыСборки["ПутьКVanessaBehavior"];
	Иначе
		Стр = Стр + " /Execute " + ПараметрыСборки["ПутьКОбработкеЧтобыСохранитьЗамерПроизводительности"];
	КонецЕсли;	 
	Стр = Стр + " /C""StartFeaturePlayer;VBParams=" + ФайлПараметров.ПолноеИмя +  """ /TESTMANAGER ";
